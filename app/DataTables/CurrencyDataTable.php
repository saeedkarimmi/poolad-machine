...
class CurrencyDataTable extends BaseDataTable implements DatabaseInterface
{
    /**
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                return sprintf('<a href="%s" title="%s"><i class="fa fa-edit text-navy"></i></a>',
                    route('panel.currencies.edit', $row->id), __('general.form.edit'));
            });
    }
    ...
    protected function getBuilderParameters()
    {
        ...
        array_unshift($params['buttons'], [
            'text' => 'ایجاد ارز جدید',
            'className' => 'action',
            'action' => 'function(e, dt, node, config) { var u = "' . route('panel.currencies.create') . '"; window.location.href = u; }'
        ]);
        return $params;
    }
}
